...
- **Yardım** (`/yardim`): ekran ekran kullanım rehberi + **Sürüm Geçmişi sekmesi** (`?sekme=surum`; v0.22 güncellemeleri dahil). Mobil portal kartında tıklayınca büyüyen ekran görüntüleri var (`public/yardim/*.webp`).
- **Yedekleme & Geri Yükleme** (`npm run backup`, `npm run restore`): 17 tablodan **16'sını** (`university_rankings` hariç — script'ten yeniden üretilebilen referans veri) ve `student-photos` storage bucket'ını tarihli JSON + görsel yedeği olarak indirir. `.github/workflows/backup.yml` ile her gece saat 05:00 TSİ otomatik çalışır, istenirse gizli (private) GitHub reposuna aktarır.
